...
EPS_DECAY = 0.99 # per-episode decay: eps <- max(EPS_MIN, eps * EPS_DECAY)
...
bins=create_bins()
def expected_q_next(Q:np.ndarray,s_next:tuple,eps:float):
    q_row=Q[s_next]
    max_q=float(np.max(q_row))
    mean_sum=float(np.sum(q_row))
    return (1-eps)*max_q+eps*mean_sum/2
    ...
    for ep in range(1,N_EPISODES+1):
        ...
        while not done:
            ...
            if not done:
                ...
                exp=expected_q_next(Q,s_next,eps)
                #best_next=Q[s_next+(a_next,)]    
                td_target=r+GAMMA*exp
